...
of the site. Under a seeded random sample of the same size the share came out at
...
from rebootpcl.checks.check5_missingness_scale import run, COMP_EXPLAINS_FLAG
...
def main():
    ...
    print("=" * 78)
    print("CHECK 5 — sampling sensitivity of the composition share")
    print("=" * 78)
    print(f"n={args.n} stays per arm | flag threshold {COMP_EXPLAINS_FLAG}\n")

    print("legacy alphabetical prefix (how the 0.49 headline was produced):",
          flush=True)
    leg = positive(run(seed=0, n=args.n, legacy=True))
    print(f"  explained={leg.stats['explained']:.3f}  "
    ...
    for s in range(args.seeds):
        ...
        shares.append(c.stats["explained"])
        flags.append(bool(c.flagged))
        print(f"  seed {s}: explained={c.stats['explained']:.3f}  "
              f"avail_ratio={c.stats['max_avail_ratio']:.1f}  "
              f"flagged={c.flagged}", flush=True)

    shares = np.array(shares)
    print("\n" + "-" * 78)
    print(f"legacy prefix     : {leg.stats['explained']:.3f}")
    ...
    outside = not (lo <= leg.stats["explained"] <= hi)
    print("\nverdict:", (
    ...
    with open(os.path.join(RESULTS, "check5_sampling.json"), "w",
        ...
        json.dump({"legacy_explained": leg.stats["explained"],
                   "legacy_flagged": bool(leg.flagged),
                   "random_explained": [float(x) for x in shares],
                   "random_flagged": flags,
                   "n": args.n, "threshold": COMP_EXPLAINS_FLAG}, fh, indent=2)
